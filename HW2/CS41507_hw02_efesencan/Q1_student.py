...
generators = []
all_possibilities = set(range(1, p)) # for each possible element
for chance in all_possibilities:
    check_list = set()
    for validate in all_possibilities: # take the power of the element with all possible elements in the set
        check_list.add(pow(chance, validate) % p)
    if check_list == all_possibilities: # if the iterated element generates all the elements in Zp when taking the powers, than that element is the generator
        generators.append(chance) # since we need only one generator, we break the loop when we find the first asnwer
        break
